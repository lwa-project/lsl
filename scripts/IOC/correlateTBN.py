...
def parseConfig(args):
	...
	try:
		opts, arg = getopt.getopt(args, "hql:t:s:o:24xy", ["help", "quiet", "fft-length=", "avg-time=", "samples=", "offset=", "two-products", "four-products", "xx", "yy"])
...
def main(args):
	...
	for o in project.sessions[0].observations:
		if int(mtch.group('mjd')) == o.mjd and int(mtch.group('opcode')) == o.opcode:
			...
			break
	if cObs is None:
		print "ERROR:  observation entry for file '%s' not found in '%s'" % (dataFile, metaDataFile)
		sys.exit()
