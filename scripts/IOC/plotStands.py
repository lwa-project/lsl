...
LWA-1 using a MCS meta-data bundle.  Also, mark and label particular stands, 
if requested.
...
# Command line flags - default values
	...
	# Add in arguments
	config['args'] = [arg[0],]
	config['args'].extend([int(i) for i in arg[1:]])
...
def main(args):
	...
	toMark = numpy.array(config['args'][1:])-1
	...
	# Set the opacity based on whether or not they are good
	alpha = [0.6 if ant.getStatus() == 33 else 0.3 for ant in antennas[::2]]
	...
	c = ax1.scatter(data[:,0], data[:,1], c=color, s=40.0, alpha=alpha)
