...
def main(args):
	...
	if cObs is None:
		...
		gain = project.session[0].tbnGain
		freq = cObs.frequency1
	
	nSamples = os.path.getsize(dataFile) / tbn.FrameSize
	print "Samples in file: ", nSamples
	fh = open(dataFile, "rb", buffering=tbn.FrameSize)
	...
	for i in range(nSamples):
		...
		try:
			cFrame = tbn.readFrame(fh, SampleRate=sampleRate, CentralFreq=freq, Gain=gain)
