...
class SubSubSystemStatus(object):
	...
	def __init__(self, fee=None, rpd=None, sep=None, arb=None, dp1=None, dp2=None, dr=None, ADPCompatibleStation=False):
		...
		if fee is None:
			self.fee = [0 for n in xrange(self.mcsRef.ME_MAX_NFEE)]
		else:
			self.fee = fee
		
		if rpd is None:
			self.rpd = [0 for n in xrange(self.mcsRef.ME_MAX_NRPD)]
		else:
			self.rpd = rpd
			
		if sep is None:
			self.sep = [0 for n in xrange(self.mcsRef.ME_MAX_NSEP)]
		else:
			self.sep = sep
		
		if arb is None:
			self.arb = [0 for n in xrange(self.mcsRef.ME_MAX_NARB)]
		else:
			self.arb = arb
...
class StationSettings(object):
	...
	def __init__(self, report=None, update=None, fee=None, aspFlt=None, aspAT1=None, aspAT2=None, aspATS=None, 
		...
		if fee is None:
			self.fee = [0 for n in xrange(self.mcsRef.ME_MAX_NSTD)]
		else:
			self.fee = fee
			
		if aspFlt is None:
			self.aspFlt = [0 for n in xrange(self.mcsRef.ME_MAX_NSTD)]
		else:
			self.aspFlt = aspFlt
			
		if aspAT1 is None:
			self.aspAT1 = [0 for n in xrange(self.mcsRef.ME_MAX_NSTD)]
		else:
			self.aspAT1 = aspAT1
			
		if aspAT2 is None:
			self.aspAT2 = [0 for n in xrange(self.mcsRef.ME_MAX_NSTD)]
		else:
			self.aspAT2 = aspAT2
			
		if aspATS is None:
			self.aspATS = [0 for n in xrange(self.mcsRef.ME_MAX_NSTD)]
		else:
			self.aspATS = aspATS
