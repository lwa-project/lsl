...
from astropy.coordinates import ICRS, ITRS, FK4, FK5, GeocentricTrueEcliptic, Galactic as AstroGalactic
...
# I'm not sure how to really do this
from ephem import Equatorial, Ecliptic, Galactic
...
class Equatorial(_Coordinate):
    ...
    def __init__(self, ra_or_class, dec=None, epoch=None):
        if epoch == B1950:
            __astropy_base__ = FK4
        elif epoch == J2000:
            __astropy_base__ = FK5
            
        _Coordinate.__init__(self, ra_or_class, dec, epoch=epoch, hours=True)
        
    @property
    def ra(self):
        _gc = self._coord.transform_to(ITRS(obstime=self._epoch))
        return _gc.ra.to('radian').value
        
    @property
    def dec(self):
        _gc = self._coord.transform_to(ITRS(obstime=self._epoch))
        return _gc.dec.to('radian').value
