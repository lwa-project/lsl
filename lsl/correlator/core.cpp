...
#define LAUNCH_FENGINE_COMPLEX(IterType) \
    ...
    switch( PyArray_LSL_TYPE(data, 2) ){
        ...
        case( LSL_CI8        ): LAUNCH_FENGINE_COMPLEX(int8_t); break;
...
#define LAUNCH_PFBENGINE_COMPLEX(IterType) \
    ...
    switch( PyArray_LSL_TYPE(data, 2) ){
        ...
        case( LSL_CI8        ): LAUNCH_PFBENGINE_COMPLEX(int8_t); break;
...
static PyObject *XEngine2(PyObject *self, PyObject *args) {
    ...
    // Validate the other input arrays against signals1
    if( PyArray_DIM(data2, 0) != nStand || PyArray_DIM(data2, 1) != nChan || PyArray_DIM(data2, 2) != nFFT ) {
        PyErr_Format(PyExc_RuntimeError, "signals1 and signals2 have different dimensions");
        goto fail;
    }
    if( PyArray_DIM(valid1, 0) != nStand || PyArray_DIM(valid1, 1) != nFFT ) {
        PyErr_Format(PyExc_RuntimeError, "sigValid1 dimensions do not match signals1");
        goto fail;
    }
    if( PyArray_DIM(valid2, 0) != nStand || PyArray_DIM(valid2, 1) != nFFT ) {
        PyErr_Format(PyExc_RuntimeError, "sigValid2 dimensions do not match signals1");
        goto fail;
    }
